...
Once the checks have run the output can be reviewed under https://github.com/openid/publication/actions.  Further pushes to branches that match the propose/** pattern will result in re-run of the document checks. In the event of the checks failing because of an external dependancy failure (such as availability of openid.net/specs) then it is possible to re-run jobs from the GitHub "actions" interface. Details of how to fix the various errors are documented in https://github.com/openid/publication/blob/main/ERROR-MODES.md.
